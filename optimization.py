...
class BERTAdamax(Optimizer):
    def __init__(self, params, lr=2e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay_rate=0, max_grad_norm=1.0, warmup=-1, t_total=-1, schedule='warmup_linear'):
        if not 0.0 <= lr:
        ...
        if not 0.0 <= weight_decay_rate:
            raise ValueError("Invalid weight_decay value: {}".format(weight_decay_rate))

        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay_rate, max_grad_norm=max_grad_norm,
                        warmup=warmup, t_total=t_total, schedule=schedule)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['weight_decay'] != 0:
                    grad += group['weight_decay_rate'] * p.data
